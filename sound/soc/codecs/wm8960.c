...
// by pjlee : path
static const char *playback_path[] = { "OFF", "SPK", "HP" };
static const char *voicecall_path[] = { "OFF", "SPK", "HP" };
static const char *mic_path[] = { "Main Mic" };
...
static int wm8960_get_playback_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int wm8960_set_playback_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static int wm8960_get_voicecall_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int wm8960_set_voicecall_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int path_num = ucontrol->value.integer.value[0];
...
static int wm8960_get_mic_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
// by pjlee : Path Control
SOC_ENUM_EXT("Playback Path", path_control_enum[0],
		wm8960_get_playback_path, wm8960_set_playback_path),
SOC_ENUM_EXT("Voice Call Path", path_control_enum[1],
		wm8960_get_voicecall_path, wm8960_set_voicecall_path),
SOC_ENUM_EXT("MIC Path", path_control_enum[2],
		wm8960_get_mic_path, wm8960_set_mic_path),
};
...
#if 1
	snd_soc_write(codec, WM8960_APOP1, 0x94);
	snd_soc_write(codec, WM8960_APOP2, 0x40);
	msleep(400);   /* HP output discharge */
	snd_soc_write(codec, WM8960_POWER2, 0x0);
	snd_soc_write(codec, WM8960_APOP2, 0);
	snd_soc_write(codec, WM8960_POWER1, 0x80);
	msleep(400);   /* VMID initial charge */
	snd_soc_write(codec, WM8960_POWER1, 0x142);
	snd_soc_write(codec, WM8960_APOP1, 0);
	//---------------------------------------------------------------//
	snd_soc_write(codec, WM8960_LOUT2, 0xFF);
	snd_soc_write(codec, WM8960_ROUT2, 0xFF);
	snd_soc_write(codec, WM8960_CLASSD1, 0x40);
	snd_soc_write(codec, WM8960_CLASSD3, (0x05<<3)|0x04);

//	snd_soc_set_bias_level(codec, SND_SOC_BIAS_STANDBY);

	/*  set the update bits */
	reg = snd_soc_read(codec, WM8960_LOUT1);
	snd_soc_write(codec, WM8960_LOUT1, reg | 0x0100);
	reg = snd_soc_read(codec, WM8960_ROUT1);
	snd_soc_write(codec, WM8960_ROUT1, reg | 0x0100);

	reg = snd_soc_read(codec, WM8960_LOUT2);
	snd_soc_write(codec, WM8960_LOUT2, reg | 0x0100);
	reg = snd_soc_read(codec, WM8960_ROUT2);
	snd_soc_write(codec, WM8960_ROUT2, reg | 0x0100);

	reg = snd_soc_read(codec, WM8960_LOUTMIX);
	snd_soc_write(codec, WM8960_LOUTMIX, reg | 0x0100);
	reg = snd_soc_read(codec, WM8960_ROUTMIX);
	snd_soc_write(codec, WM8960_ROUTMIX, reg | 0x0100);
#endif
			   ...
			   enum snd_soc_control_type control)
{
	...
	snd_soc_write(codec, WM8960_LOUT1, reg | 0x07f);
	reg = snd_soc_read(codec, WM8960_ROUT1);	// 03
	snd_soc_write(codec, WM8960_ROUT1, reg | 0x07f);
