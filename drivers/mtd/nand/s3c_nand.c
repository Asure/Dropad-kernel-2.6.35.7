...
static int s3c_nand_probe(struct platform_device *pdev, enum s3c_cpu_type cpu_type)
{
	...
	s3c_nand = kzalloc(sizeof(struct s3c_nand_info), GFP_KERNEL);
	if (!s3c_nand) {
		err = -ENOMEM;
		goto exit_error;
	}
	...
	s3c_nand->clk_sub = clk_get(&pdev->dev, "nandxl");
...
//	size = resource_size(res);
	...
	s3c_nand->area = request_mem_region(res->start, resource_size(res), pdev->name);
	...
	s3c_nand->regs       = ioremap(res->start, resource_size(res));
