	...
	u8 *spd_data)
{
	...
	writel(SET_SPD_HEADER(header[0]), hdmi_base + S5P_SPD_HEADER0);
	writel(SET_SPD_HEADER(header[1]), hdmi_base + S5P_SPD_HEADER1);
	writel(SET_SPD_HEADER(header[2]), hdmi_base + S5P_SPD_HEADER2);

	for (i = 0; i < 8; i++) {
		checksum += vendor[i];
		writeb(SET_SPD_DATA(vendor[i]),
			hdmi_base + S5P_SPD_DATA1 + (i * 4));
	}
	...
	for (i = 0; i < 17; i++) {
		...
		writeb(SET_SPD_DATA(product[i]),
			hdmi_base + S5P_SPD_DATA9 + (i * 4));
	}

	checksum = 0x100 - (checksum & 0xff);

	writeb(SET_SPD_DATA(checksum), hdmi_base + S5P_SPD_DATA0);
...
#ifndef CONFIG_SND_S5P_SOC_SPDIF
